...
public class myProfileWordInterface {
    DefaultTableModel list;
    JTable listTable;
    JScrollPane listScroll;
    
    myProfileWordInterface(){
        ...
        setPreferredColumnWidth(listTable, 0, 100);
        setPreferredColumnWidth(listTable, 1, 300);
        ...
        try {
            Map<String, String> ranking = database.sort("misspelled", signInFrame.username.getText().trim());
            int i = 1;
            for (Map.Entry<String, String> entry : ranking.entrySet()){
                list.addRow(new Object[]{i, entry.getKey(), String.format("%.0f", Float.parseFloat(entry.getValue()))});
                i++;
            }
        } catch(Exception e){
        }
